...
~function () {
    ...
    xhr.onreadystatechange=function () {
        if(xhr.readyState===4&&/^2\d{2}$/.test(xhr.status)){
            jsonData="JSON" in window ? JSON.parse(xhr.responseText) : eval("(" + xhr.responseText + ")");console.log(jsonData);
        }
    };
    xhr.send(null);
    function bindData(jasonData) {
      var divData=jasonData["div"],code=divData["code"];

        var str='';
        for(var i=0;i<divData.length;i++){
            str+='<div class="span16"><div class="tab-container"><ul class="tab-content clearfix">';
            for (var j=0;j<code.length;j++){
               var curLi=code[j];
                if(i<7){
                    ...
                    str+='<div class="reviver-wrapper"><a href="javascript:;"><span class="reviver">'+curLi["reviver"]+'</span><span class="autor">'+curLi["autor"]+'</span></a></div>';
                    str+='</li>';
        ...
        var oJrow=document.getElementById("J_row");
        oJrow.innerHTML+=str;
    }
...
!function () {
    ...
    var oLis=JMatchList.getElementsByTagName("li");
    var JMatchItem=document.getElementById("J_match_item");
    var oDivs=DOM.getByClass("span16",JMatchItem);
    for (var i=0;i<oLis.length;i++){
        var curLi=oLis[i];
        curLi.index=i;
